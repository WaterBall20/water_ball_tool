...
impl WBFPManager {
    pub(super) fn get_file_pos(&mut self, length: u64) -> io::Result<(u64, u64)> {
    ...
    pub(super) fn file_gc(&mut self) -> io::Result<()> {
        let pack_file = self.pack_file.clone();
        let mut pack_file = pack_file
            .lock()
            .map_err(|err| Error::other(format!("无法获得包文件锁, err: {err}")))?;
    ...
    pub(super) fn manifest_file_gc(&mut self) -> io::Result<()> {
