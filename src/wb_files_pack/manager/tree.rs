...
impl WBFPManager {
        ...
        if path_list.len() > 1 {
            ...
            while let Some(name) = s_name {
                ...
                if let Some(item) = s_pack_struct.items().get(name) {
                    if let Some(next_name) = name_list.next() {
                        match &item.item_type() {
                            ...
                            PackStructItemType::File { .. } => {
                                Err(PackFileError::NotADirectory(format!(
                                    r#"虚拟路径"{}"是文件不是目录"#,
                                    this_path.display()
                                )))?;
                            }
                    ...
                    Err(PackFileError::NotFound(format!(
                        r#"虚拟路径目录"{}"的结构项不存在"#,
                        this_path.display()
                    )))?;
                }
    ...
    pub(crate) fn load_all_data(&mut self, no_err: bool) -> Result<()> {
            ...
            for name in keys {
                ...
                if let PackFileMetadataRun::NoLoad = item.metadata() {
                    ...
                    let metadata = match wbfp_manager.load_pack_file_metadata(metadata_file_pos) {
                        ...
                        Err(err) => Err(PackFileError::State(format!(
        ...
        if let Err(err) = m_load_all_data(self, &mut temp_ps, no_err, &PathBuf::new())
            && !no_err
        {
            for name in &root_keys {
                let item = temp_ps
                    .remove_item(name)
                    .ok_or(PackFileError::State(format!("临时结构项\"{name}\"不存在")))?;
    ...
    pub(crate) fn load_pack_struct_metadata_path<P: AsRef<Path>>(&mut self, path: P) -> Result<()> {
    ...
    fn load_pack_struct_metadata_path2(
        ...
        let two_name = path_list.next().ok_or(PackFileError::State(
            "路径列表为空，无法加载结构元数据".into(),
        ))?;
        let two_pack_struct =
            ...
            while let Some(this_name) = name {
                ...
                if let Some(item) = pack_struct.items().get(this_name) {
                    let this_pack_struct = match &item.item_type() {
                        ...
                        PackStructItemType::File { .. } => Err(PackFileError::NotADirectory(
