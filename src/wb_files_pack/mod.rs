...
//已分配数据列表项
//位置
static DATA_POS_S_ITEM_POS_LEN: usize = 8;
//长度
static DATA_POS_S_ITEM_LEN_LEN: usize = 8;
//总大小
static DATA_POS_LIST_ITEM_LEN: usize = DATA_POS_S_ITEM_POS_LEN + DATA_POS_S_ITEM_LEN_LEN;
...
impl DataPosList {
    fn load(data: &[u8]) -> io::Result<Self> {
        ...
        let data_pos_s_data = &data
            [DATA_POS_LIST_COUNT_LEN..DATA_POS_LIST_COUNT_LEN + (count * DATA_POS_LIST_ITEM_LEN)];
        let mut list = Vec::with_capacity(count);
        while list.len() < count {
            let index = list.len();
            let pos = u64::from_le_bytes(
                data_pos_s_data[(index * DATA_POS_LIST_ITEM_LEN)
                    ..(index * DATA_POS_LIST_ITEM_LEN) + DATA_POS_S_ITEM_POS_LEN]
                    .try_into()
                    .unwrap(),
            );
            let len = u64::from_le_bytes(
                data_pos_s_data[(index * DATA_POS_LIST_ITEM_LEN) + DATA_POS_S_ITEM_POS_LEN
