use crate::wb_files_pack::{
    DataPosList, ManifestDataBlock, DATA_BLOCK_LEN, MANIFEST_ATTRIBUTE_LEN,
};
use std::fs::File;
use std::io;
...
//文件头块长度
pub(in crate::wb_files_pack) const FILE_HEADER_BLOCK_LEN: usize = DATA_BLOCK_LEN;
...
pub(crate) struct RunData {
    ...
    //GC数据列表
    pub(crate) gc_data_pos_list: DataPosList,
}
...
impl PackIO {
    ...
    //垃圾回收
    pub(crate) fn file_gc(&mut self) -> io::Result<()> {
        //准备：排序
