use crate::command::ff::search_files;
use crate::command::{create_pb, set_pb_style2, update_pb, BUF_LEN, PROGRESS_STYLE_TEMPLATE};
...
use water_ball_tool::file_finder::{FileInfo, FileKind, FilesList};
...
pub fn wbfp(args: WaterBallFilePackArgs, mp: Option<&MultiProgress>) {
    ...
    match args2 {
        WaterBallFilePackCommands::Unpack(u) => wbfp_s(u, mp),
        WaterBallFilePackCommands::Pack(p) => wbfp_m(p, mp),
        WaterBallFilePackCommands::HashVerify(h) => wbfp_h(h, mp),
    }
}
/// 打包目录为水球包文件。
///
...
pub fn wbfp_m(args: &WaterBallFilePackCommandsPack, mp: Option<&MultiProgress>) {
    //源目录路径
    let in_dir_path = &args.in_path;
    //输出的包文件路径
