...
const TEST_DIR: &str = "./temp/test/file_finder/ok";
...
fn search_dir(path: &Path, skip_symlink: bool) -> FilesList {
    ...
    for _ in rx {}
    crate::tools::TestTool::expect_ok(
...
fn create_test_file(dir: &Path, name: &str) -> PathBuf {
    ...
    let mut f = crate::tools::TestTool::expect_ok(fs::File::create(&path), "创建测试文件失败");
    crate::tools::TestTool::expect_ok(f.write_all(b"test content"), "写入测试文件内容失败");
    path
}
...
fn symlink_to_file_is_found() {
    let root = setup_test_dir("symlink_to_file_is_found");
    ...
    crate::tools::TestTool::cleanup_test_dir(&root);
}
...
fn symlink_to_dir_is_traversed() {
    let root = setup_test_dir("symlink_to_dir_is_traversed");
    let sub = root.join("subdir");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&sub), "创建子目录 subdir 失败");
...
fn symlink_to_ancestor_is_detected() {
    let root = setup_test_dir("symlink_to_ancestor_is_detected");
    let sub = root.join("sub");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&sub), "创建子目录 sub 失败");
...
fn multi_level_symlinks_work() {
    let root = setup_test_dir("multi_level_symlinks_work");
    let dir_a = root.join("a");
    let dir_b = root.join("b");
    let dir_c = root.join("c");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&dir_c), "创建子目录 c 失败");
...
fn skip_symlinks_flag_works() {
    let root = setup_test_dir("skip_symlinks_flag_works");
    let file = create_test_file(&root, "real.txt");
...
fn broken_symlink_does_not_crash() {
    let root = setup_test_dir("broken_symlink_does_not_crash");
...
fn mixed_real_and_symlinks() {
    let root = setup_test_dir("mixed_real_and_symlinks");
    create_test_file(&root, "real_file.txt");
    let real_dir = root.join("real_dir");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&real_dir), "创建真实目录 real_dir 失败");
...
fn same_target_via_different_chains() {
    let root = setup_test_dir("same_target_via_different_chains");
    let target = root.join("target_dir");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&target), "创建目标目录 target_dir 失败");
...
fn chain_propagates_correctly() {
    let root = setup_test_dir("chain_propagates_correctly");
    let level1 = root.join("level1");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&level1), "创建 level1 目录失败");
    let level2 = level1.join("level2");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&level2), "创建 level2 目录失败");
...
fn search_stream_receives_all_entries() {
    let root = setup_test_dir("search_stream_receives_all_entries");
    create_test_file(&root, "a.txt");
    create_test_file(&root, "b.txt");
    let sub = root.join("sub");
    crate::tools::TestTool::expect_ok(fs::create_dir_all(&sub), "创建流式测试子目录 sub 失败");
    create_test_file(&sub, "c.txt");

    // search_stream 返回 SearchEvent 接收器
    let (tx, rx) = mpsc::channel();
    let (handle, stream_rx) = crate::tools::TestTool::expect_ok(
        FileFinder.search_stream(&root, false, tx, 4),
        "启动流式搜索失败",
    );
...
fn search_stream_empty_dir() {
    let root = setup_test_dir("search_stream_empty_dir");
...
/// 随机目录树搜索压力测试：随机文件数/大小/目录深度，验证搜索完整性
/// Randomized directory-tree search stress test: random file count/size/
/// directory depth, verifying search completeness
///
/// 随机夹具是测试产物（随机规范）：仅在 `temp/test/` 下生成，非资源文件；
/// 预算内（≤1000 文件、单文件 ≤4MiB、深度 ≤5）；失败保留现场文件即复现。
/// Randomized fixtures are test artifacts (random strategy): generated only
/// under `temp/test/`, never resource files; within budget (≤1000 files,
/// single file ≤4MiB, depth ≤5); leftover files at the failure site ARE the
/// reproduction.
#[test]
#[ignore = "longtime"]
fn random_tree_search_stress() {
    let root = setup_test_dir("random_tree_search_stress");

    // 广度优先生成目录树：每层每目录 0-3 个子目录 + 1-8 个随机大小文件，
    // 深度最多 4 层（根为第 0 层）。预估上限约 968 文件（121 目录 × 8）。
    let mut dirs: Vec<PathBuf> = vec![root.clone()];
    let mut expected_files = 0u32;
    for depth in 0..4 {
        let mut next: Vec<PathBuf> = Vec::new();
        for d in &dirs {
            let sub_count = rand::random_range(0..=3);
            for k in 0..sub_count {
                let sub = d.join(format!("rand_sub_{depth}_{k}"));
                crate::tools::TestTool::expect_ok(fs::create_dir_all(&sub), "创建随机子目录失败");
                next.push(sub);
            }
            let file_count = rand::random_range(1..=8);
            for k in 0..file_count {
                let size = rand::random_range(1..=2048);
                let content: Vec<u8> = (0..size).map(|_| rand::random::<u8>()).collect();
                let fpath = d.join(format!("rand_file_{depth}_{k}.bin"));
                crate::tools::TestTool::expect_ok(fs::write(&fpath, &content), "写入随机文件失败");
                expected_files += 1;
            }
        }
        dirs = next;
    ...
    assert!(expected_files > 0, "随机树不应为空");
